...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MultiProvider(
      providers: [ChangeNotifierProvider(create: (_) => HospitalProvider())],
      child: MaterialApp(
...
class HospitalProvider extends ChangeNotifier {
    ...
    _filteredHospitals =
        _hospitals.where((hospital) {
          ...
          if (_searchQuery.isNotEmpty) {
            ...
            final specialtyMatch = hospital.specialties.any(
              (specialty) => specialty.toLowerCase().contains(query),
            );
            final districtMatch = hospital.district.toLowerCase().contains(
              query,
            );
            return nameMatch || addressMatch || specialtyMatch || districtMatch;
          }
  ...
  List<Hospital> getNearbyHospitals(
    double userLat,
    double userLng, {
    double radiusKm = 50,
  }) {
