...
class _HospitalCardState extends State<HospitalCard>
  ...
  Widget _buildSpecialties() {
    final displaySpecialties =
        widget.hospital.specialties.take(3).toList(); // Reduced from 4
  ...
  Widget _buildActionButtons() {
    return Row(
      children: [
        ...
        // Book Slot / View Booking Button
        Expanded(
          child: FutureBuilder<bool>(
            future: _isHospitalBooked(widget.hospital.id),
            builder: (context, snapshot) {
              final isBooked = snapshot.data ?? false;

              if (isBooked) {
                // Show "Booked" button that allows viewing booking
                return Container(
                  height: 36,
                  decoration: BoxDecoration(
                  ...
                  child: Material(
                    color: Colors.transparent,
                    child: InkWell(
                      onTap: () => _viewBooking(widget.hospital.id),
                      borderRadius: BorderRadius.circular(10),
                      child: Row(
                        mainAxisAlignment: MainAxisAlignment.center,
                        children: [
              ...
              } else if (widget.hospital.availableBeds > 0) {
  ...
  Future<bool> _isHospitalBooked(String hospitalId) async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final bookedHospitals = prefs.getStringList('booked_hospitals') ?? [];
      return bookedHospitals.contains(hospitalId);
    } catch (e) {
      return false;
    }
  }

  Future<void> _viewBooking(String hospitalId) async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final bookingId = prefs.getString('booking_${hospitalId}');
